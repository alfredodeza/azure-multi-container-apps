//Use actix_web to expose a REST API endpoint called /tokenize that accepts a JSON payload with a text field and returns a JSON payload with a tokens field.
//The tokens field should be an array of strings, where each string is a token from the text field.
//The tokens should be generated using the tokenizer crate.
//The tokenizer crate should be used as a dependency in your Cargo.toml file.
